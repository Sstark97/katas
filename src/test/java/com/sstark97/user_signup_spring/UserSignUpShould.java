...
class UserSignUpShould {
    ...
    void return_400_when_sign_up_with_a_bad_email_format() throws Exception {
        UserSignUpRepository repository = Mockito.mock(UserSignUpRepository.class);
        UserSignUpService signUpService = new UserSignUpService(repository);
        UserSignUp user = new UserSignUp("name", "badFormatEmail", "@Passw0rd");
        String userToJson = mapper.writeValueAsString(user);

        Mockito.when(repository.save(user)).thenReturn("The email have a bad format");
