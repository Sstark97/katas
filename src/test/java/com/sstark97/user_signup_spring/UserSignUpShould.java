...
class UserSignUpShould {
    ...
    void return_400_when_sign_up_and_email_already_exists() throws Exception {
        ...
        String userToJson = mapper.writeValueAsString(user);
        ApiError theseEmailAlreadyExist = new ApiError("This email already exist", HttpStatus.BAD_REQUEST);
    ...
    @Test
    void return_400_when_password_not_have_any_digit() throws Exception {
        UserDto user = new UserDto("name", "email@email.com", "Password");
        String userToJson = mapper.writeValueAsString(user);
        ApiError passwordWithIncorrectLength = new ApiError("The password must have 1 digit or more", HttpStatus.BAD_REQUEST);
